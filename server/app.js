// Entry point for the Anima backend. Postgres via Supabase (no more raw
...
const mediaUnitsRoutes = require('./api/media_units.js');
const arcsRoutes = require('./api/arcs.js');
const progressRoutes = require('./api/progress.js');
const mediaListRoutes = require('./api/media_list.js');
const mediaStaffRoutes = require('./api/media_staff.js');
...
app.use('/', mediaUnitsRoutes);
app.use('/', arcsRoutes);
app.use('/', progressRoutes);
app.use('/', mediaListRoutes);
app.use('/', mediaStaffRoutes);
app.use('/', mediaRoutes); // media routes last: it has the catch-all /:category routes

app.listen(port, () => {
  console.log(`Anima API running on port ${port}`);
});
